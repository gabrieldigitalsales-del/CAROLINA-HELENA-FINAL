...
database_url = os.getenv("DATABASE_URL", "sqlite:///" + os.path.join(BASE_DIR, "instance", "loja.db"))
...
DEFAULT_SETTINGS = {
    ...
    "contact_image": "images/about-cover.svg",
}
...
def admin_settings():
    if request.method == "POST":
        ...
        if contact_image and contact_image.filename:
            path = save_upload(contact_image)
            if path:
                set_setting("contact_image", path)
